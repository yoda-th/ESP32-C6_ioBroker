...
static void mqttOnMessage(char* topic, byte* payload, unsigned int length) {
    ...
    if (String(topic) == TOPIC_CMD && commandCallback) commandCallback(msg);
}
...
void mqttPublishEvent(const String &eventName, const String &extraJson) {
    ...
    // Das JSON Format, das dein ioBroker erwartet
...
void mqttPublishState(const String &s) {
    if(mqttClient.connected()) mqttClient.publish(TOPIC_STATE, ("{\"state\":\""+s+"\"}").c_str(), true);
}
...
void mqttLogDataPoint(String j) {
    ...
    else if(historyQueue.size() < 100) historyQueue.push_back(j);
}
