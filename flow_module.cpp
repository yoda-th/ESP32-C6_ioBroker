...
static const float PULSES_PER_LITER = 450.0f; // Standard China Sensor
...
void IRAM_ATTR flowIsr() {
    ...
    if (nowUs - lastPulseMicros < MIN_PULSE_SPACING_US) return; // Entprellen
...
void flowLoop() {
    ...
    // 100ms Fenster (Deine Logik)
    if (now - lastCalcMs >= 100) {
        unsigned long pulses = pulseCount;
        pulseCount = 0; // Reset für nächstes Intervall

        // Overflow Schutz
        if (pulses > MAX_PULSES_PER_INTERVAL) {
            // logWarn("Flow Glitch ignored"); // Spam vermeiden
            pulses = 0; 
        }
        ...
        // Plausibilitäts-Filter (Deine V0.9 Logik)
        if (candidateLpm > MAX_LPM_ABS) candidateLpm = MAX_LPM_ABS;
        
        // Sprung-Filter (nur wenn Flow schon lief)
        if (lastLpm > 0.1f) {
